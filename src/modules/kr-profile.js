...
async function unpublishProfileDeck(group, button) {
  ...
  try {
    await marketApi(`/my-decks/${encodeURIComponent(meta.remoteId)}/unpublish`, { method: 'PATCH' });
    await syncMyMarketDeckMetadata();
    renderProfile();
    toast('牌组已下架，不再公开展示。');
  } catch (error) {
