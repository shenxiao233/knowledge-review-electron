 ...
 * Dependencies: kr-core.js, kr-state.js, markdown-renderer.js (markdownToHtml), kr-cards.js (cardHtml)
 * Provides: saveDoc, loadDoc, renderKnowledgeHome, renderTree, switchDoc,
 *           duplicateTreeItem, exportDocument, deleteDoc, outline, editorCommand,
 *           renderLatexInHtml, openImageLightbox, insertImage, handleEditorPaste
 */
function saveDoc() { const doc = activeDoc(); if (!doc) return; doc.html = restoreLatexForStorage(els.noteEditor.innerHTML); doc.updatedAt = new Date().toISOString(); saveDocChange(doc.id); save(); }
...
async function insertImage() { const url = await openInputDialog('插入图片', '图片地址', 'https://'); if (!url) return; const caption = await openInputDialog('插入图片', '图片注释（可选）', ''); focusEditorSelection(); document.execCommand('insertHTML', false, `<figure class="editor-image"><img src="${esc(url)}" alt="插入图片"><figcaption contenteditable="true">${esc(caption || '')}</figcaption></figure><p><br></p>`); saveDoc(); outline(); }
...
function pinDocument(docId) { const doc = state.documents.find((item) => item.id === docId); if (!doc) return; doc.pinned = !doc.pinned; saveDocChange(docId); save(); refresh({ tree: true, home: true }); toast(doc.pinned ? '文章已置顶。' : '已取消置顶。'); }
...
function moveDocumentFromModal(event) { event.preventDefault(); const doc = state.documents.find((item) => item.id === actionTarget?.id); if (!doc) return; const name = $('#moveFolderSelect').value; doc.folderId = state.folders.find((folder) => folder.name === name)?.id || null; saveDocChange(doc.id); save(); $('#moveDocumentModal').close(); refresh({ tree: true, home: true }); toast('文章已移动。'); }
...
function moveDoc(docId, folderId) { const doc = state.documents.find((item) => item.id === docId); if (!doc || doc.folderId === folderId) return; doc.folderId = folderId || null; saveDocChange(docId); save(); renderTree(); toast('文档已移动。'); }
...
function deleteDoc(docId) { const doc = state.documents.find((item) => item.id === docId); if (!doc) return; openDeleteConfirm('document', docId, `删除文档“${doc.title}”？`, '文档将被永久删除，此操作无法撤销。'); }
function deleteFolder(folderId) { const folder = state.folders.find((item) => item.id === folderId); if (!folder) return; const count = state.documents.filter((doc) => doc.folderId === folderId).length; openDeleteConfirm('folder', folderId, `删除文件夹“${folder.name}”？`, count ? `其中 ${count} 篇文档将随文件夹永久删除，此操作无法撤销。` : '文件夹将被永久删除，此操作无法撤销。'); }
